...
public class Produit extends ClassEtat{
    ...
    public void insererDansBoutique(Connection conn) throws Exception {
        this.setIdTypeProduit("TPR000081");
        this.createObject("1060", conn);
    }
    
    public void insererDansBoutique() throws Exception{
        Connection conn = new UtilDB().GetConn();
        
        this.insererDansBoutique(conn);
    ...
    public static Produit rechercherProduitParId(String idProduit, Connection conn) throws Exception {
        if(idProduit == null || idProduit.trim().isEmpty()) throw new Exception("Id du produit est requis pour effectuer la recherche!");
        if(conn == null) throw new Exception("La connection est null: Produit.rechercherParId(String, Connection)"); 
        
        Produit[] produits = (Produit[])CGenUtil.rechercher(new Produit(), null, null, conn, " AND ID LIKE '" + idProduit + "'");
        if(produits.length == 0) throw new Exception("Aucun produit trouvee: " + idProduit);
