...
//        if(venteDetails == null) System.out.println("Vente erreur");
        double totalMGA = 0.00;
        for(VenteDetails vd : venteDetails) {
            totalMGA += vd.getMontant();
        }
        this.setMontantht(totalMGA);
...
//        valeur.setIdAvoirFC(venteDetails.get());
    ...
    public static AvoirFCFille transformerFactureToAvoirFille(VenteDetails venteDetails, double qte) throws Exception{
        AvoirFCFille valeur = new AvoirFCFille();
...
//        valeur.setIdAvoirFC(venteDetails.get());
        ...
        valeur.setIdOrigine(venteDetails.getIdOrigine());
        valeur.setQte(qte);
        valeur.setPu(venteDetails.getPuVente() * qte);
        valeur.setMontantht(venteDetails.getPuVente() * qte);
        valeur.setMontantttc(venteDetails.getPuVente() * qte);
    ...
    public static Vente[] rechercherVenteBoutique() throws Exception {
        try (Connection conn = new UtilDB().GetConn()) {
            return rechercherVenteBoutique(conn);
        } catch (Exception e) {
            e.printStackTrace();
            throw e;
        }
    ...
    public static Vente[] rechercherVenteBoutique(Connection conn) throws Exception {
        return (Vente[])CGenUtil.rechercher(new Vente(), null, null, conn, " AND IDMAGASIN LIKE 'MAG000245'ORDER BY DATY");
    }
    
    public void rechercherParId(Connection conn) throws Exception {
        if(this.getId() == null || this.getId().trim().isEmpty()) throw  new Exception("L'Id de la vente est null");
        
        Vente[] ventes = (Vente[]) CGenUtil.rechercher(this, null, null, conn, " AND ID LIKE '" + this.getId() + "'");
        
        if(ventes.length == 0) throw new Exception("Aucune vente ayant ID: " + this.getId());
        this.setDaty(ventes[0].getDaty());
        this.setDesignation(ventes[0].getDesignation());
    }
//    public VenteDetails[] getVenteDetails(Connection conn) throws Exception {
//        return (VenteDetails[])CGenUtil.rechercher(new VenteDetails(), null, null, conn, " AND IDVENTE LIKE '" + this.getId() + "'");
//        
//    }
