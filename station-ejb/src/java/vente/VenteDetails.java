...
//    }
    ...
    public static VenteDetails rechercherParId(String id, Connection conn) throws Exception {
        VenteDetails[] vds = (VenteDetails[]) CGenUtil.rechercher(new VenteDetails(), null, null, conn, " AND ID LIKE '" + id + "'");
        if(vds.length == 0) throw new Exception("Aucun vente details trouvee: " + id);
        return vds[0];
    } 
    
     public void annuler(Connection conn, double qte) throws Exception {
//         Avoi
//        Avoir
        conn.setAutoCommit(false);
        Vente vente = Vente.getById(conn, this.getIdVente());
        
        AvoirFC avoir = Vente.transformerFactureToAvoir(vente);
        AvoirFCFille avoirFille = Vente.transformerFactureToAvoirFille(this, qte);
        avoir.createObject("1060", conn);
        avoirFille.setIdAvoirFC(avoir.getId());
        avoirFille.createObject("1060", conn);
//        avoir.setFille(new Av);
...
//        AvoirFC avoir = new AvoirFC();
        ...
        mvtStockFille.setEntree(qte);
        mvtStockFille.setSortie(0.00);
